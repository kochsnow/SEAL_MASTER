...
def parse_args():
    ...
    parser.add_argument("--B", type=int, default=500,
                        help="bugets for active  learning")
    parser.add_argument("--T", type=int, default=50,
                        help="iteration times for active  learning")
...
def ai_mask(model,features, train_mask, val_mask):
    model.eval()
    ...
    lam = int(args.B / args.T)
    chosen_index = np.argsort(sub_scores, axis = 0)[-lam:]
    print(lam, chosen_index)
...
def train_ai(g, model, cl_model=None):
    train_mask = g.ndata['train_mask'].bool()
    val_mask = g.ndata['val_mask'].bool()
    train_mask_indexs = (val_mask == True).nonzero(as_tuple=False).numpy().reshape(-1,).tolist()
    train_mask[train_mask_indexs[500:1000]] = False
    val_mask = g.ndata['val_mask'].bool()
    val_mask[train_mask_indexs[500:1000]] = True
    test_mask = g.ndata['test_mask'].bool()
    features = g.ndata['feature']
    train(g, model, train_mask, val_mask, test_mask, 200, cl_model)
    for t in tqdm(range(args.T), total=args.T):
        train(g, model, train_mask, val_mask, test_mask, args.epoch, cl_model)
        ai_mask(model, features, train_mask, val_mask)
    train(g, model, train_mask, val_mask, test_mask, 200, cl_model)


def train(g, model, train_mask, val_mask, test_mask,epochs, cl_model=None):
